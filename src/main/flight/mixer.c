...
static void applyMixToMotors(float motorMix[MAX_SUPPORTED_MOTORS], motorMixer_t *activeMixer)
{
    ...
    for (int i = 0; i < mixerRuntime.motorCount; i++)
    {
        ...
        // HOVECRAFT ADD condition here if hovercraftMode apply channel forward.
#ifdef HOVERCRAFT_MODE
        if (i == 2 || i == 3)
        {
            motorOutput = (rcData[AUX5] - 1000) * 2;
        }
#endif
...
FAST_CODE_NOINLINE void mixTable(timeUs_t currentTimeUs)
{
    ...
    if (featureIsEnabled(FEATURE_MOTOR_STOP) && ARMING_FLAG(ARMED) && !mixerRuntime.feature3dEnabled && !airmodeEnabled && !FLIGHT_MODE(GPS_RESCUE_MODE) // disable motor_stop while GPS Rescue is active
